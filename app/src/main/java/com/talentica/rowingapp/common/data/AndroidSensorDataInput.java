...
public class AndroidSensorDataInput extends SensorDataInputBase {
	...
	public AndroidSensorDataInput(DashBoardActivity owner) {
	...
	private int getSensorDelay(DashBoardActivity owner) {
		int sensorDealyMillis = Integer.valueOf(owner.preferencesHelper.getPref("com.talentica.rowing.android.sensor.sensorUpdateDelay", SENSOR_DELAY + ""));
	...
	private class SensorDataThread extends CompatHandlerThread implements SensorEventListener  {
		...
		protected void onLooperPrepared() {
			...
			Log.i("onLooperPrepared","attaching accelSensor listener at notification delay of {} "+ sensorDelay);
//			logger.debug("attaching accelSensor listener at notification delay of {}", sensorDelay);
			...
			Log.d("onLooperPrepared","attaching orientSensor listener at notification delay of {} "+ sensorDelay);
//			logger.debug("attaching orientSensor listener at notification delay of {}", sensorDelay);
		...
		public void onSensorChanged(SensorEvent event) {
			...
			case Sensor.TYPE_ACCELEROMETER:
				...
				if (firstAccelEvent) {
					Log.d("onSensorChanged","received first TYPE_ACCELEROMETER data ({})"+ values);
			...
			case Sensor.TYPE_ORIENTATION:
				if (firstOrientEvent) {
					Log.d("onSensorChanged","received first TYPE_ORIENTATION data ({})"+ values);
