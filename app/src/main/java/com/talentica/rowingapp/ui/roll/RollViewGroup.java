...
public class RollViewGroup extends LinearLayout implements DataUpdatable {
	...
	public RollViewGroup(Context context, AppStroke roboStroke) {
		super(context);
	...
	public void setMode(ViewType mode) {
		...
		for (RollView view: rollViews) {
			...
			((LayoutParams)view.getLayoutParams()).rightMargin = 0;
		}
		...
		for (RollView view: views) {
			...
			if (++i < views.length) {
				((LayoutParams)view.getLayoutParams()).rightMargin = rightMargine;
			}
