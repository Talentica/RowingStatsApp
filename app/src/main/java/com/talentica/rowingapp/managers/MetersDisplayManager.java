...
import com.talentica.rowingapp.common.data.stroke.RowingSplitMode;
...
import com.talentica.rowingapp.ui.utils.LayoutMode;
...
public class MetersDisplayManager implements SensorDataSink {
    ...
    private long startTime;//timestamp origin for global time
    private long splitTimeStart;//timestamp start for split time
    ...
    private final DashBoardActivity owner;
     ...
     */
    public MetersDisplayManager(final DashBoardActivity owner) {
        this.owner = owner;
        ...
        splitTimeTxt.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                bus.fireEvent(DataRecord.Type.ROWING_START_TRIGGERED, true);
            }
        });
...
//                        Toast.makeText(owner, "ROWING_START_TRIGGERED", Toast.LENGTH_SHORT).show();
                        triggered = (Boolean) event.data;
                        break;
                    case ROWING_START:
//                        Toast.makeText(owner, "ROWING_START", Toast.LENGTH_SHORT).show();
                        Log.i("ROWING_START {}", String.valueOf(event.timestamp));
                        triggered = false;
                        splitTimerOn = true;
                        ...
                        break;
                    case ROWING_STOP:
//                        Toast.makeText(owner, "ROWING_STOP", Toast.LENGTH_SHORT).show();
                        Log.i("ROWING_STOP {}", String.valueOf(event.timestamp));
                        ...
                        break;
                    case ROWING_COUNT:
//                        Toast.makeText(owner, "ROWING_COUNT", Toast.LENGTH_SHORT).show();
                        lastStrokeCount++;
                        updateCount();
                        break;
                    case STROKE_POWER_END:
//                        Toast.makeText(owner, "STROKE_POWER_END", Toast.LENGTH_SHORT).show();
                        hasPower = (Float) event.data > 0;
                        Log.i("STROKE_POWER_END", " (has power: {})" + hasPower);

                        break;
                    case STROKE_RATE:
                        if (hasPower) {
                            int spm = (Integer) event.data;
//                            Toast.makeText(owner, "STROKE_RATE:"+spm, Toast.LENGTH_SHORT).show();
                            spmAccum += spm;
                            spmCount++;
                            updateSpm(spm);
                            hasPower = false;
                    ...
                    case BOOKMARKED_DISTANCE: {
                        ...
                        Log.i("BOOKMARKED_DISTANCE", "elapsedDistance = {}" + splitDistance);
//                        Toast.makeText(owner, "BOOKMARKED_DISTANCE:"+splitDistance, Toast.LENGTH_SHORT).show();
...
//                        Toast.makeText(owner, "WAY:"+distance, Toast.LENGTH_SHORT).show();
    ...
    private void updateLayout(LayoutMode meterLayout) {
        ...
        if (currentMeterLayout != nextMeterLayout) {
            int[] containers = {R.id.spm_meter_container, R.id.speed_meter_container,
                    R.id.distance_meter_container, R.id.time_meter_container};
